...
- **Admin panel** (`/admin`, or `admin.html` directly): password-gated, with three tabs:
  - **Products** — add/edit/delete products (name, category, price, MRP, sizes, colors, rating, review count, image URL, description, optional 360° spin photo URLs, trending flag), plus a search box to filter the product table by name/category.
...
- **Interactive product viewer (360° spin / 3D tilt)**: the product detail page's main image is drag-interactive instead of a static photo. If a product has 2+ `spinImages` set (photos shot in a circle around the garment, added via the admin panel's "360° Spin Photo URLs" field), dragging left/right steps through those frames for a true 360° rotation, with a "360°" badge shown on the image. Every product today only has one stock photo, so by default dragging instead does a 3D tilt (CSS `perspective` + `rotateX`/`rotateY` following the drag) on that single photo — an honest fallback that doesn't pretend to show angles we don't have photos of. Both modes show a "Drag to rotate/tilt" hint badge that fades out after first interaction. See `initProductViewer()` in `js/script.js`.
...
**Test mode (no Razorpay keys needed):** until `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` are set, `/api/checkout/create-order` no longer blocks checkout with a 503 — instead it creates the booking directly with a simulated "paid" status (order/payment ids prefixed `TEST_ORDER_...`/`TEST_PAYMENT_...`, real prices still recomputed server-side). This lets you exercise the whole pipeline — register/sign in → reserve → see it appear in the admin "Bookings & Payments" tab → change its tracking status → watch that update show up on the customer's "My Orders" page — without needing real Razorpay credentials yet. Test bookings are visibly tagged (a black "TEST" badge in admin, a "TEST MODE" note on the customer's order card and the cart success screen) and are excluded from the admin dashboard's revenue stat cards so they never look like real money collected. This path disappears automatically and permanently the moment real `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` are set — there's no separate flag to flip back.
...
- **Test-mode bookings**: `formatBooking()` in `server.js` derives `is_test` by checking whether `razorpay_order_id` starts with `TEST_` — don't rename that prefix without updating that check (and the matching ones in `js/admin.js`'s badge logic and `js/script.js`'s order card). `renderBookingStats()` in `js/admin.js` explicitly filters `is_test` bookings out of the revenue numbers; if you add more admin stats later, remember to exclude test bookings there too or they'll inflate the numbers with fake money.
- **360° spin / 3D tilt viewer**: `products.spin_images` (JSON array column in `db.js`, default `'[]'`) holds optional photo URLs for a true 360° spin; `formatProduct()`/`validateProductBody()` in `server.js` expose/accept it as `spinImages`. The admin form field is a plain comma-separated textarea (`#p-spin-images` in `admin.html`, wired in `startEdit()`/`submitForm()` in `js/admin.js`) — no upload pipeline, admins paste already-hosted image URLs shot in a circle around the garment, in order. On the frontend, `initProductViewer()` in `js/script.js` picks the mode automatically per product: 2+ `spinImages` → real frame-stepping rotation on drag; fewer than 2 (true for every seed product today) → CSS 3D tilt on the single existing photo instead. Both paths share one drag handler (Pointer Events, with a mouse/touch fallback for older browsers) so don't fork the event-binding logic when tweaking one mode — see the comment block above `initProductViewer()` for the exact math. To add real 360° photography for a product later: shoot 24–36 frames in a circle, host them anywhere, paste the URLs into that admin field — no code changes needed.
